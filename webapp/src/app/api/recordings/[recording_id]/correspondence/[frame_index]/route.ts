...
export async function POST(
  ...
  try {
    ...
    console.log(`[API Proxy] Processing correspondence request for recording: ${recording_id}, frame: ${frame_index}`);
    ...
    const targetUrl = `${DATA_SERVICE_URL}/recordings/${recording_id}/correspondence/${frame_index}`;

    console.log(`[API Proxy] Forwarding correspondence request to: ${targetUrl}`);
    ...
    try {
      const backendResponse = await fetch(targetUrl, {
        ...
        cache: 'no-store', // Ensure fresh results for correspondence
      });
      ...
      const matchResults = await backendResponse.json();
      console.log("[API Proxy] Received match results from backend:", matchResults);
    ...
    console.error('[API Proxy] Unexpected error in correspondence route handler:', outerError);
    return NextResponse.json({
